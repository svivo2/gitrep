...
function computeMaxArr(f1, f2) {
	...
	//  returns the largest number.  So, for a three element array arr, the output
	...
	var maxArr = function(a) {
		...
		else if ((f1(a[a.length - 1])) > (f2(a[a.length - 1]))) {
			arr.unshift(f1(a[a.length - 1]));
			a.pop();
			return maxArr(a);
		}
		else {
			arr.unshift(f2(a[a.length - 1]));
			a.pop();
			return maxArr(a);
		}
	}
